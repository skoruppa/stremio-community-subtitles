...
def addon_stream(manifest_token: str, content_type: str, content_id: str, params: str = None):
    ...
    current_app.logger.info(f"Subtitle request: User={user.username}, Lang={preferred_lang}, Content={content_type}/{content_id}, Hash={video_hash}, Size={video_size}, Filename={video_filename}")
    ...
    try:
        ...
        if video_hash is not None and video_size is not None:
            ...
            if existing_activity:
                ...
                current_app.logger.info(f"Updated existing UserActivity ID {existing_activity.id} (match by hash/size) for user {user.id}, hash {video_hash}, size {video_size}")
            ...
            if existing_activity:
                ...
                current_app.logger.info(f"Updated existing UserActivity ID {existing_activity.id} (match by filename, no hash/size) for user {user.id}, filename {video_filename}")
                activity_found_and_updated = True
        ...
        if not activity_found_and_updated:
            ...
            current_app.logger.info(f"Created new UserActivity for user {user.id}, content {content_id}, hash {video_hash}, size {video_size}, filename {video_filename}")
    ...
    try:
        ...
        download_identifier = base64.urlsafe_b64encode(context_json.encode('utf-8')).decode('utf-8').rstrip('=')  # Remove padding
    except Exception as e:
...
def unified_download(manifest_token: str, download_identifier: str):
    ...
    if video_hash:
        ...
        if user_selection_specific and user_selection_specific.selected_subtitle:
            ...
            current_app.logger.info(f"Using user selection (specific hash): Subtitle ID {found_subtitle.id} for content {content_id}, hash {video_hash}")
    ...
    if not found_subtitle and not video_hash:
        ...
        if user_selection_general and user_selection_general.selected_subtitle:
            ...
            current_app.logger.info(f"Using user selection (general content): Subtitle ID {found_subtitle.id} for content {content_id}")

    # 2. If NO applicable user selection was found, proceed to default lookup logic
    if not found_subtitle:
        current_app.logger.info(f"No applicable user selection found for content {content_id}, proceeding to default lookup.")
        ...
        if preferred_lang:  # Proceed only if preferred language is known
            ...
            if video_hash:
                ...
                subs_without_hash = base_query.filter(Subtitle.video_hash == None).order_by(Subtitle.votes.desc()).first()
        ...
        try:
            ...
            try:
                ...
                if existing_selection:
                    ...
                    current_app.logger.info(f"Updated UserSubtitleSelection for user {current_user.id}, content {activity.content_id}, hash {activity.video_hash} to subtitle {subtitle.id}")
                else:
                    ...
                    current_app.logger.info(f"Created UserSubtitleSelection for user {current_user.id}, content {activity.content_id}, hash {activity.video_hash} for subtitle {subtitle.id}")
...
def select_subtitle(activity_id, subtitle_id):
    ...
    try:
        ...
        if not current_user.has_role('Admin') and not user_selection and not (active_subtitle and active_subtitle.id == subtitle_id):
...
def delete_subtitle(subtitle_id):
    ...
    try:
        ...
        if subtitle.file_path:
            file_path = os.path.join(current_app.config['UPLOAD_FOLDER'], subtitle.file_path)
            if os.path.exists(file_path):
                try:
                    os.remove(file_path)
                except Exception as e:
                    current_app.logger.error(f"Error deleting subtitle file {file_path}: {e}")
